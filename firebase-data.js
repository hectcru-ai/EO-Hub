...
// One-time: pushes two specific content corrections into already-seeded
// Firestore kit documents. Editing kits_data.js alone never reaches these —
// that file only seeds NEW kits, it doesn't update existing ones.
export async function applyKitContentFixes(onProgress){
  const fixes = [
    {
      kitId: '31',
      updates: {
        contents: ["C300 MKII Camcorder body (EF Mount)", "Canon Battery Pack BP-A30 (amount noted per-unit)", "Battery Charger", "Compact Power Adapter", "Handle Unit", "Handle Bracket", "Shoulder Strap (on bag)", "Eye cup", "Monitor", "Side Handle", "Audio Module", "Ballhead Mount", "Canon Strap"],
      },
    },
    {
      kitId: '62',
      updates: {
        contents: ["EKP AVX Receiver (blue tip)", "SK AVX Transmitter (Body Pack) w/Belt Clip", "Sennheiser NT 5-10 UW Switching Power Supply OR Belkin Universal Power Adapter (only one included)", "Mic w/1/8\" end", "Windscreen", "Mic Clip", "(1) USB to Micro USB Cable"],
        batteryNote: "🔋 BA 20 and BA 30 batteries are separate Cheqroom inventory items, not part of this kit — check the original reservation and add them at checkout if they weren't already requested.",
      },
    },
  ];
  let done = 0;
  for(const fix of fixes){
    await updateKit(fix.kitId, fix.updates);
    done++;
    if(onProgress) onProgress(done, fixes.length);
  }
  return done;
}
...
export async function applyBatch4Updates(onProgress){
  const batch = [
    // ── Grade tier assignments from Sept 2 2026 review ──────────────
    ...
    {id:'59', review:'Confirmed: batteries are Zhiyun GMB-B117 18650 (replacement batteries, not original BTW INR).'},
  ];
...
export async function addLooseItemReport(report){
  const ref = doc(collection(db, 'looseItemReports'));
  await setDoc(ref, { id: ref.id, ...report, reported: serverTimestamp(), resolved: false });
  return ref.id;
}
export async function getLooseItemReports(){
  const snap = await getDocs(collection(db, 'looseItemReports'));
  return snap.docs.map(d => d.data()).sort((a,b)=>(b.reported?.seconds||0)-(a.reported?.seconds||0));
}
export async function updateLooseItemReportStatus(id, status){
  // status: 'reviewed' | 'archived'
  await updateDoc(doc(db, 'looseItemReports', id), { status, resolved: status !== 'open' });
}

export async function addActionTrackingEntry(data){
  const ref = doc(collection(db, 'actionTracking'));
  await setDoc(ref, { id: ref.id, ...data, invoiced:false, crHold:false, regHold:false, paid:false, accessRestored:false, status:'open', created: serverTimestamp() });
  return ref.id;
}
export async function getActionTrackingEntries(){
  const snap = await getDocs(collection(db, 'actionTracking'));
  return snap.docs.map(d => d.data()).sort((a,b)=>(b.created?.seconds||0)-(a.created?.seconds||0));
}
export async function updateActionTrackingEntry(id, updates){
  await updateDoc(doc(db, 'actionTracking', id), updates);
}

export async function clearResolvedReviewFlags(onProgress){
  const ids = [
    '9',  // ARRI 650w Kit — scrim labeling confirmed non-issue
    '20', // China Ball Kit — Cabe confirmed contents
    '21', // Scrim Jim Cine Kit — retired
    '47', // Libec Tripod — confirmed active and tiered
    '53', // Dana Dolly Track Kit — lowboys/SJ stands confirmed correct
    '56', // VariZoom QuickJib — naming resolved
    '64', // Wired Lavalier(s) — confirmed active
    '79', // Sidus Link Bridge — confirmed active
    '80', // Tentacle Sync Timecode Generator — confirmed active
    '81', // Cinebags AC Pouch — confirmed active
    '86', // E-Image Matte Box — Sophomore tier confirmed
  ];
  for(let i=0; i<ids.length; i++){
    await updateKitData(ids[i], { review: '' });
...
const MAX_VIDEOS_PER_KIT = 4;

function migrateVideosField(kit){
  // Older kits may still have the single-video field from before this
  // was multi-video. Treat that as video #1 rather than losing it.
  if(kit.videos) return kit.videos;
  if(kit.videoUrl) return [kit.videoUrl];
  return [];
}

export async function getKitVideos(kitId){
  const kit = await getKit(kitId);
  return kit ? migrateVideosField(kit) : [];
}

export async function addKitVideoLink(kitId, videoUrl){
  const kit = await getKit(kitId);
  const videos = migrateVideosField(kit);
  if(videos.length >= MAX_VIDEOS_PER_KIT){
    throw new Error(`This kit already has the maximum of ${MAX_VIDEOS_PER_KIT} videos — remove one before adding another.`);
  }
  videos.push(videoUrl);
  await updateKit(kitId, { videos, videoUrl: null });
  return videos.length - 1;
}

export async function removeKitVideoAt(kitId, index){
  const kit = await getKit(kitId);
  const videos = migrateVideosField(kit);
...
export async function applyBatteryNotes(onProgress){
  // 🔋 Battery notes for all kits that need them.
  // "amount noted per unit" = count varies per physical unit, check label.
  // Standard = batteries included, verify they are charged.
  // "not included" = only the mount/plate is in the kit, actual battery reserved separately.
  const batch = [
    // ── Amount noted per unit ──────────────────────────────────────────────
    {id:'83', batteryNote:'🔋 Canon Battery Pack BP-955 — count varies per unit. Check the unit number label before checkout and confirm the correct amount.'},
    {id:'31', batteryNote:'🔋 Battery charger included. Battery count may vary per unit — verify count on the unit label before checkout.'},
    {id:'36', batteryNote:'🔋 BP-U960 batteries — quantity varies per unit. Verify the count on the unit before checkout.'},
    {id:'34', batteryNote:'🔋 V-Mount battery plate included. Actual battery count varies per unit — check the unit before checkout.'},
    // ── Batteries included — verify charged ────────────────────────────────
    {id:'67', batteryNote:'🔋 NP-F770 batteries are in this kit — ensure they are fully charged before checkout.'},
    {id:'32', batteryNote:'🔋 Includes NP-F570 battery and V-Mount plate — verify both are charged before checkout.'},
    {id:'35', batteryNote:'🔋 4 Sony a7S II batteries are in this kit — verify all are charged before checkout.'},
    {id:'26', batteryNote:'🔋 LP-E8 battery included — verify it is charged before checkout.'},
    {id:'27', batteryNote:'🔋 LP-E10 battery included — verify it is charged before checkout.'},
    {id:'29', batteryNote:'🔋 EN-EL9 battery included — verify it is charged before checkout.'},
    {id:'62', batteryNote:'🔋 BA 20 and BA 30 batteries for the wireless transmitters are in this kit — verify spares are present and charged.'},
    {id:'69', batteryNote:'🔋 8 batteries are inside the radios — verify all are charged before checkout.'},
    {id:'6',  batteryNote:'🔋 NPF batteries included — verify they are charged before checkout.'},
    {id:'71', batteryNote:'🔋 NP-F750 batteries included — verify charged before checkout.'},
    {id:'72', batteryNote:'🔋 Battery charger included — verify batteries are charged before checkout.'},
    {id:'77', batteryNote:'🔋 Sony NPF batteries included — verify charged before checkout.'},
    {id:'78', batteryNote:'🔋 Sony NPF batteries included — verify charged before checkout.'},
    {id:'75', batteryNote:'🔋 V-Mount battery included — verify it is charged before untethered use.'},
    {id:'76', batteryNote:'🔋 V-Mount battery included — verify it is charged before untethered use.'},
    {id:'59', batteryNote:'🔋 6 Zhiyun GMB-B117 18650 batteries included — verify all are charged before checkout.'},
    {id:'58', batteryNote:'🔋 NPF battery plate included — verify battery is charged before checkout.'},
    {id:'65', batteryNote:'🔋 MixPre-6 II recorder batteries — NP-F770 included. Verify charged before checkout.'},
    // ── Battery NOT included — must be reserved separately ─────────────────
    {id:'49', batteryNote:'🔋 Gold Mount battery mount is in the kit but the battery itself is NOT included — student must reserve a V-Mount or Gold Mount battery separately.'},
  ];
