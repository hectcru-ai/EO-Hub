...
export async function setKitContents(kitId, contents){
  // Read-modify-write to reliably replace a kit's contents array
  const ref = doc(db, 'kits', String(kitId));
  const snap = await getDoc(ref);
  if(!snap.exists()) throw new Error(`Kit ${kitId} not found`);
  const existing = snap.data();
  await setDoc(ref, { ...existing, data: { ...existing.data, contents } });
}
