...
export async function uploadKitPhoto(kitId, dataUrl){
  ...
  // Stored directly in the kit's own Firestore document rather than
  // Firebase Storage — Storage requires the paid Blaze plan, and this
  // avoids that entirely. Firestore caps a whole document at 1MiB, so
  // there's a real ceiling here; the resize/compression this gets put
  // through before calling this function keeps it comfortably under that.
  if(dataUrl.length > 900000){
    throw new Error('This photo is too large to store. Try a smaller or simpler image.');
  }
  await updateKit(kitId, { photoUrl: dataUrl });
  return dataUrl;
}
...
export async function removeKitPhoto(kitId){
  // Clears the photo and its labels together — labels are positioned
  // relative to a specific photo, so they don't carry over to a new one.
  await updateKit(kitId, { photoUrl: null, hotspots: [] });
}
...
export async function setKitHotspots(kitId, hotspots){
  await updateKit(kitId, { hotspots });
}
