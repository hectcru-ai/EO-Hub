export const CHEQROOM_SECTIONS = [
  {
    id: 99,
    title: "The Full Order Lifecycle: Building → Checkout → Check-In → Approvals",
    officialLink: null,
    updated: "Staff Manual",
    body: `
  <p>This is the same real workflow new hires get trained on, start to finish. Each stage has its own detailed click-by-click steps further down this list — this section is the big picture tying them together, not a replacement for those.</p>
  <ol>
    <li><strong>Checking Professor Approval emails</strong> — this often has to happen before an order can move forward. Check the Equipment Office inbox's Professor Approvals folder, print the approval as a PDF, attach it to the matching Cheqroom order, and flip the status from Pending Professor Approval to Approved. Also check for a COI/Production Permit while you're in there, and attach those too. If a request is denied, change the status accordingly and let the student know to follow up with their professor — that's on them to resolve, not EO staff. (An email template for missing paperwork is included in that section below.)</li>
    <li><strong>Building the order</strong> — pull items in priority order: Professor/Special Project orders first, then the soonest student order. Build the combined checklist in the EO Hub app (Check Out-In → New Checkout), pull everything onto a cart, and stage it in the Check-Out Area. If your shift ends before it's done, log an update in the weekly order dashboard so the next shift knows exactly where things stand — don't leave it half-built with no explanation.</li>
    <li><strong>Checkout</strong> — the student physically verifies items against the checklist in the EO Hub app, staff adds any last essentials in Cheqroom (never extras), and the Checkout Agreement gets printed and wet-signed.</li>
    <li><strong>Check-in</strong> — check items in manually, verify function item by item, and flag anything missing or damaged — that becomes a follow-up email, not an on-the-spot argument with the student.</li>
  </ol>
  <div class="callout">If any of these four detailed steps looks different from what you were actually trained on, that's worth flagging to the Manager — the detailed sections below are meant to be the single source of truth, so a real mismatch needs fixing there, not just remembered as "how we actually do it."</div>
`,
  },
  {
    id: 100,
...
`,
  ...
  <div style="margin-top:18px; border-top:1px solid var(--line-soft); padding-top:16px;">
    <div style="font-weight:700; font-size:13px; margin-bottom:6px;">📧 Email Template — Missing Paperwork (Reservation Approaching)</div>
    <div style="font-size:12.5px; color:var(--text-muted); margin-bottom:10px;">Use this when a student's pickup date is coming up and we still haven't received the required approval or permits. Copy the text below and paste it into a new email.</div>
    <div style="background:var(--bg); border:1px solid var(--line); border-left:4px solid var(--amber); border-radius:6px; padding:12px 14px; font-size:13px; font-family:monospace; white-space:pre-wrap; line-height:1.6; user-select:all;">Subject: Action Required — Missing Paperwork for Equipment Reservation #[RESERVATION #]

Dear [Student Name],

Your equipment reservation (#[RESERVATION #]) is scheduled for pickup on [DATE]. As we begin preparing your order, we noticed that we have not yet received the required documentation to process it.

To avoid any delay or cancellation of your reservation, please contact Admin Services as soon as possible to ensure all necessary forms and approvals are submitted prior to your pickup date:

    Ryland Murphy
    Logistics Manager, Administrative Services
    ADM 116 — 1000 Regent University Drive, Virginia Beach, VA 23464
    757.352.4034 | rylamur@regent.edu

If you have any questions or believe this message was sent in error, please don't hesitate to reach out to us directly at equipment@regent.edu.

Thank you,
Equipment Office
Regent University</div>
  </div>
`,
  },
  {
  ...
  <ol>
    ...
    <li>Review the order in Cheqroom before pulling anything: check for missing essentials (C-Fast cards, batteries). Note anything missing in the Cheqroom order comments so whoever checks the student out knows to add it.</li>
    ...
    <li>The checkout checklist is now built in the EO Hub app — staff builds it under <strong>Check Out-In → New Checkout</strong> before the student's pickup date. Place the printed Cheqroom liability form on the cart with the order.</li>
    <li>Stage the finished cart in the Check-Out Area with the Cheqroom liability form.</li>
  </ol>
  <div class="callout">Only build 1–2 days out. Never build an order 3+ days ahead. <strong>If you can't finish building an order during your shift, log an update in the weekly order dashboard</strong> so the next shift knows exactly where things stand — don't leave it half-done with no explanation.</div>
`,
  },
  {
    id: 107,
